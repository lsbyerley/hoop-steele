...
router.get('/games/:date*?', cors(corsOptions), cache(100), async (req, res) => {
    ...
    if (gamesData) {
      gamesData.forEach((game, i) => {
        ...
        const gameOdds = {
          spread: _get(game, 'competitions[0].odds[0].details'),
          total: _get(game, 'competitions[0].odds[0].overUnder')
        }
        const status = {
        ...
        if (prediction && _get(game, 'competitions[0].odds[0]')) {

          let vegasTotal = parseFloat(gameOdds.total)
          let predictedTotal = parseFloat(prediction.totalOutput)
          let totalDiff = (vegasTotal > predictedTotal) ? vegasTotal - predictedTotal : predictedTotal - vegasTotal
          totalDiff = round(totalDiff, 1)

          let spreadDiff = 0
          let split = gameOdds.spread.split(' ')
          if (split.length == 2) {
